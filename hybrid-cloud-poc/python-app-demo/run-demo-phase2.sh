#!/bin/bash
# Unified-Identity - Phase 1 & Phase 2: Complete demo script with Real Keylime Verifier
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Unified-Identity - Phase 1 & Phase 2: Python App Demo         ║"
echo "║  Using Real Keylime Verifier (Phase 2)                        ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
echo "ℹ This demo uses the real Keylime Verifier with Phase 2 implementation"
echo ""
...
# Stop processes using Phase 2 stop script
...
echo -e "${BOLD}${CYAN}Step 1: Setting up SPIRE and Real Keylime Verifier (Phase 2)...${RESET}"
echo ""

# Check if Phase 2 is available
if [ ! -d "${PHASE2_DIR}" ]; then
    echo -e "${RED}Error: Phase 2 directory not found at ${PHASE2_DIR}${RESET}"
    exit 1
fi
...
# Start SPIRE and Keylime using Phase 2 script
...
echo "    3. Server calls Real Keylime Verifier (Phase 2)"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  Keylime Verifier Logs - Unified-Identity (Phase 2)           ║"
echo "╚════════════════════════════════════════════════════════════════╝"
if [ -f /tmp/keylime-verifier.log ]; then
    grep -i "unified-identity.*phase 2" /tmp/keylime-verifier.log | tail -15 || echo "⚠ No Unified-Identity Phase 2 logs found"
else
