...
## Optimized Implementation & Rollout Plan

The goal is to complete the easiest-to-stub parts first, which are the API changes and the policy logic, before tackling the complex, hardware-dependent TPM/cryptography.

### Phase 1: SPIRE API & Policy Staging (Stubbed Keylime)

**Goal:** Implement all necessary SPIRE API changes and the policy logic in the SPIRE Server without relying on a functional Keylime or TPM plugin.

| Component | Action | Test Coverage |
| :--- | :--- | :--- |
| **SPIRE Server** | Implement the new `X509SVIDRequest` logic. | **Unit/Integration Test:** Policy engine successfully evaluates fixed/stubbed `AttestedClaims` and issues an SVID with the correct extensions, or triggers remediation. |
| **SPIRE Agent** | Implement the new `X509SVIDRequest` flow. | **Unit Test:** Agent successfully populates the `SovereignAttestation` with fixed/stubbed nonces and quotes/certificates for flow testing. |
| **Keylime Verifier** | **STUB:** Create a mock Keylime Verifier API that validates the mTLS call and always returns a fixed, hardcoded `AttestedClaims` response (e.g., geolocation: Spain, integrity: PASSED). | **Integration Test:** SPIRE Server successfully authenticates to and receives the fixed facts from the mock Keylime API. |

### Phase 2: Core Keylime Functionality (Fact-Provider Logic)

**Goal:** Fully implement the Keylime Verifier's logic to validate a full PoR/PoG request. Use fixed, pre-generated keys for testing.

| Component | Action | Test Coverage |
| :--- | :--- | :--- |
| **Keylime Verifier** | Implement the full validation logic for `POST /v2.4/verify/evidence`: <br> 1. Validate the App Key Certificate signature chain. <br> 2. Verify the TPM Quote signature using the App Key. <br> 3. Validate the nonce and return real (non-stubbed) facts from its internal store. | **Keylime Unit Test:** Full cryptographic verification tests using pre-generated keys, certificates, and quotes. |
| **Keylime Agent** | **STUB:** For now, the Agent only needs to serve the Keylime Verifier its internal fact data. The delegated certification is still isolated. | N/A |

### Phase 3: Hardware Integration & Delegated Certification

**Goal:** Implement the complex, multi-component TPM-related cryptographic flows, including the crucial delegated certification.

| Component | Action | Test Coverage |
| :--- | :--- | :--- |
| **SPIRE TPM Plugin** | Implement the generation of the TPM Quote based on the challenge nonce. | **Unit Test:** TPM plugin correctly generates quotes and App Key Public Keys. |
| **Keylime Agent** | Implement the high-privilege side of the Delegated Certification (2a) flow: securely access the AK context to sign the App Key Certificate and return it to the SPIRE Agent. | **Integration Test:** SPIRE Agent &leftrightarrow; Keylime Agent local API test for certificate generation. |
| **SPIRE Agent** | Implement the low-privilege side of the Delegated Certification (2a) flow and integrate the final `app_key_certificate` into the `SovereignAttestation` payload. | **Integration Test:** Agent successfully gets the certificate and sends a complete, cryptographically valid `SovereignAttestation` to the Server. |

### Phase 4: Full End-to-End Integration

**Goal:** Connect all fully implemented components and perform the final E2E flow testing.
...
This 4-phase approach is superior because:
- **Phase 1** isolates all SPIRE API and policy logic testing.
- **Phase 2** isolates the complex Keylime Verifier cryptography and fact retrieval logic.
- **Phase 3** isolates the most sensitive security and hardware-dependent part: App Key certification and TPM quote generation.

By structuring the work this way, you ensure that you don't chase bugs across three separate components simultaneously.
