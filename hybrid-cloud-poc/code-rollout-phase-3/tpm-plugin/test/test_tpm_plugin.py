...
class TestTPMPlugin(unittest.TestCase):
    ...
    @patch('tpm_plugin.subprocess.run')
    def test_generate_quote_stub(self, mock_run):
        """Test TPM Quote generation (stubbed)"""
        # Unified-Identity - Phase 3: Hardware Integration & Delegated Certification
        mock_result = MagicMock()
        mock_result.returncode = 0
        mock_result.stdout = ""
        ...
        # Mock file operations
        with patch('builtins.open', unittest.mock.mock_open(read_data=b"test quote data")):
            with patch('pathlib.Path.exists', return_value=True):
                plugin.generate_quote("test-nonce-12345678", pcr_list=[0, 1, 2])

        # Ensure PCR list includes hash algorithm prefix
        called_cmd = mock_run.call_args[0][0]
        self.assertIn("sha256:0,1,2", called_cmd)
