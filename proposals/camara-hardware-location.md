...
### Proposed Workflow:
1.  **Device:** Generates a "Unified SVID" containing a TPM Quote (Hardware ID) + Signed GPS Data (Location ID).
2.  **App:** Calls the CAMARA API, embedding this SVID in the request header (e.g., `X-Device-Attestation`).
3.  **CAMARA Gateway:** Forwards the token to the **AegisSovereignAI Verifier**.
4.  **Verification:**
    * *Check 1:* Is the TPM signature valid? (Anti-Cloning)
    * *Check 2:* Is the GPS data signed by the trusted sensor? (Anti-Spoofing)
    * *Check 3:* Is the device physically within the cell tower's range? (Cross-Verification)
5.  **Response:** Returns `true` only if **BOTH** the Network Location and Hardware Proof match.
...
## 6. Proof of Concept
A reference implementation of this architecture is currently available in the **[AegisSovereignAI Hybrid Cloud PoC](../hybrid-cloud-poc)**.
