...
class RegistrarServer(Server):
    def _setup(self):
        ...
        # Read num_workers from config (default to 0 = cpu_count if not specified)
        # Setting to 1 reduces memory usage from ~948 MB to ~75 MB
        self._worker_count = config.getint("registrar", "num_workers", fallback=0)
