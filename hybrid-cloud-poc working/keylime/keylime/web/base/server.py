...
class Server(ABC):
    ...
    def start_multi(self) -> None:
        ...
        # When worker_count is 1, run in single-process mode to save memory
        # This avoids the parent+child fork overhead (~75 MB saved)
        if self.worker_count == 1:
            logger.info("Running in single-process mode (num_workers=1)")
