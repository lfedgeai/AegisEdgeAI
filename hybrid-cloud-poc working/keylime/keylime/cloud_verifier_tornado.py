...
class VerifyEvidenceHandler(BaseHandler):
    ...
    def _tpm_app_key_verify(self, data: Dict[str, Any], metadata: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if not agent_mtls_cert or agent_mtls_cert == 'disabled':
            # Agent cert unavailable - try HTTPS with verifier's client cert but skip server verification
            # The rust-keylime agent requires mTLS (client cert), but we may not have the agent's cert to verify
            logger.warning('Unified-Identity: Agent mTLS certificate unavailable; using HTTPS with client cert (no server verification)')
            try:
                # Get verifier's client certificate and key
                (cert, key, _trusted_ca, key_password), _ = web_util.get_tls_options('verifier', is_client=True, logger=logger)
                if cert and key:
                    ssl_context = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)
                    ssl_context.check_hostname = False
                    ssl_context.verify_mode = ssl.CERT_NONE  # Don't verify agent's cert
                    ssl_context.load_cert_chain(certfile=cert, keyfile=key, password=key_password)
                    logger.info('Unified-Identity: Using verifier client cert for agent mTLS (no server verification)')
                else:
                    logger.warning('Unified-Identity: Verifier client cert not available; falling back to HTTP')
                    use_https = False
            except Exception as e:  # noqa: BLE001
                logger.warning('Unified-Identity: Failed to build fallback mTLS context: %s; falling back to HTTP', e)
                use_https = False
                ssl_context = None
        else:
            try:
            ...
            for api_version in api_versions:
                ...
                async def _make_request() -> Any:
                    ...
                    if use_https and ssl_context:
                        # Unified-Identity: Bypass SSL verification for self-signed agent certificates
                        # The rust-keylime agent typically uses a self-signed certificate which the
                        # verifier may not trust by default. Disable verification for agent communication.
                        ssl_context.check_hostname = False
