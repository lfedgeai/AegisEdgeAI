...
class DelegatedCertificationClient:
    ...
    def __init__(self, endpoint: str = None):
        ...
        # Check if agent mTLS is disabled via environment variable
        agent_mtls_disabled = os.environ.get("KEYLIME_AGENT_ENABLE_AGENT_MTLS", "true").lower() in ("false", "0", "no")
        
        if endpoint is None:
            if agent_mtls_disabled:
                endpoint = "http://127.0.0.1:9002"
                logger.info("Unified-Identity: Using HTTP endpoint (mTLS disabled)")
            else:
                endpoint = "https://127.0.0.1:9002"
        elif endpoint == "unix:///tmp/keylime-agent.sock":
            # Convert old UDS default to HTTP/HTTPS based on mTLS setting
            if agent_mtls_disabled:
                endpoint = "http://127.0.0.1:9002"
                logger.info("Unified-Identity: Converting old UDS default to HTTP endpoint (mTLS disabled)")
            else:
                endpoint = "https://127.0.0.1:9002"
                logger.info("Unified-Identity: Converting old UDS default to HTTPS endpoint (agent uses mTLS)")
        elif endpoint.startswith("http://") and ("127.0.0.1" in endpoint or "localhost" in endpoint):
            # Only convert HTTP to HTTPS if mTLS is enabled
            if not agent_mtls_disabled:
                endpoint = endpoint.replace("http://", "https://")
                logger.info("Unified-Identity: Converting HTTP to HTTPS (agent uses mTLS)")
            else:
                logger.info("Unified-Identity: Using HTTP endpoint (mTLS disabled)")
        ...
        if endpoint.startswith("unix://"):
            ...
            if not os.path.exists(self.socket_path):
                ...
                # Fallback to HTTP/HTTPS based on mTLS setting
                self.use_uds = False
                self.socket_path = None
                self.http_endpoint = "http://127.0.0.1:9002" if agent_mtls_disabled else "https://127.0.0.1:9002"
            ...
            if not os.path.exists(self.socket_path):
                logger.warning(
                    "Unified-Identity: UDS socket path does not exist: %s, falling back to HTTP/HTTPS",
                    self.socket_path,
                )
                # Fallback to HTTP/HTTPS based on mTLS setting
                self.use_uds = False
