...
class TPMPluginHTTPHandler(BaseHTTPRequestHandler):
    ...
    def handle_request_certificate(self, request_data: dict):
        ...
        try:
            ...
            # Unified-Identity - Verification: Default endpoint (check AGENT_MTLS_DISABLED env var)
            ...
            if not endpoint or endpoint == "unix:///tmp/keylime-agent.sock":
                if agent_mtls_disabled:
                    ...
                    logger.info("Unified-Identity - Verification: Using HTTP endpoint (mTLS disabled): %s", endpoint)
                else:
