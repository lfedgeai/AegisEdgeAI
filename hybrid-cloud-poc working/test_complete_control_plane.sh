...
# Set registrar config to use registrar.conf.minimal (which has num_workers = 1 for memory optimization)
export KEYLIME_REGISTRAR_CONFIG="${KEYLIME_DIR}/registrar.conf.minimal"
echo "    Config: ${KEYLIME_REGISTRAR_CONFIG}"
...
if [ -f "${SERVER_CONFIG}" ]; then
    ...
    # With Unified-Identity, set agent_ttl to 600s (10 minutes) to prevent re-attestation during integration tests
    # Re-attestation can cause agent ID to change, breaking registration entries
    # Note: 5 minutes was insufficient for longer test runs (wait times, propagation delays, etc.)
    if [ "${UNIFIED_IDENTITY_ENABLED:-true}" = "true" ] || [ "${UNIFIED_IDENTITY_ENABLED:-true}" = "1" ] || [ "${UNIFIED_IDENTITY_ENABLED:-true}" = "yes" ]; then
        if grep -q "Unified-Identity" "$SERVER_CONFIG" 2>/dev/null || [ -n "${SPIRE_AGENT_SVID_RENEWAL_INTERVAL:-}" ]; then
            echo "    Configuring agent_ttl for Unified-Identity (600s to prevent re-attestation during test)..."
            configure_spire_server_agent_ttl "${SERVER_CONFIG}" "600" || {
