...
export TPM_PLUGIN_CLI_PATH="${SCRIPT_DIR}/tpm-plugin/tpm_plugin_cli.py"
...
# This uses standard libraries/tools which play nicer with TPM Resource Manager
export USE_TPM2_QUOTE_DIRECT=0
echo "    ✓ USE_TPM2_QUOTE_DIRECT=0 (using standard TCTI to avoid conflicts)"
...
if [ "$SECURE_MOUNTED" = false ] && sudo -n true 2>/dev/null; then
    ...
    if [ -n "${TCTI:-}" ]; then
        sudo env -i PATH="$PATH" HOME="$HOME" USER="$USER" UNIFIED_IDENTITY_ENABLED=true USE_TPM2_QUOTE_DIRECT=0 TCTI="$TCTI" KEYLIME_DIR="$KEYLIME_AGENT_DIR" KEYLIME_AGENT_KEYLIME_DIR="$KEYLIME_AGENT_DIR" KEYLIME_AGENT_CONFIG="$TEMP_CONFIG" KEYLIME_AGENT_RUN_AS="$KEYLIME_AGENT_RUN_AS" "$(pwd)/target/release/keylime_agent" > /tmp/rust-keylime-agent.log 2>&1 &
    else
        sudo env -i PATH="$PATH" HOME="$HOME" USER="$USER" UNIFIED_IDENTITY_ENABLED=true USE_TPM2_QUOTE_DIRECT=0 KEYLIME_DIR="$KEYLIME_AGENT_DIR" KEYLIME_AGENT_KEYLIME_DIR="$KEYLIME_AGENT_DIR" KEYLIME_AGENT_CONFIG="$TEMP_CONFIG" KEYLIME_AGENT_RUN_AS="$KEYLIME_AGENT_RUN_AS" "$(pwd)/target/release/keylime_agent" > /tmp/rust-keylime-agent.log 2>&1 &
    fi
    RUST_AGENT_PID=$!
    ...
    # Use env to ensure clean environment with only the variables we need
    # Force use of TPM Resource Manager to avoid conflicts/crashes
    # Using direct /dev/tpm0 access (USE_TPM2_QUOTE_DIRECT=1) was likely causing the crash
    if [ -c /dev/tpmrm0 ]; then
        export TCTI="device:/dev/tpmrm0"
    ...
    # Use stdbuf to unbuffer logs
    sudo stdbuf -oL -eL env -i PATH="$PATH" HOME="$HOME" USER="$USER" UNIFIED_IDENTITY_ENABLED=true USE_TPM2_QUOTE_DIRECT=0 TCTI="$TCTI" KEYLIME_DIR="$KEYLIME_AGENT_DIR" KEYLIME_AGENT_KEYLIME_DIR="$KEYLIME_AGENT_DIR" KEYLIME_AGENT_CONFIG="$TEMP_CONFIG" KEYLIME_AGENT_RUN_AS="$KEYLIME_AGENT_RUN_AS" "$(pwd)/target/release/keylime_agent" > /tmp/rust-keylime-agent.log 2>&1 &
    
    RUST_AGENT_PID=$!
    echo "    Started agent with PID: $RUST_AGENT_PID (TCTI=$TCTI)"
    
    # Wait a moment to check if it crashes immediately
    sleep 5
    if ! kill -0 $RUST_AGENT_PID 2>/dev/null; then
        echo -e "${RED}    ✗ Agent crashed shortly after start! Checking logs...${NC}"
        tail -n 30 /tmp/rust-keylime-agent.log
        exit 1
    fi
else
    ...
    # Force use of TPM Resource Manager (abrmd/tpmrm0)
    if [ -c /dev/tpmrm0 ]; then
        export TCTI="device:/dev/tpmrm0"
    elif command -v tpm2-abrmd >/dev/null; then
         export TCTI="tabrmd:bus_name=com.intel.tss2.Tabrmd"
...
AGENT_CONFIG="${PROJECT_DIR}/python-app-demo/spire-agent.conf"
    ...
    if [ -f "${SPIRE_SERVER}" ] && [ -S "${SERVER_SOCKET}" ]; then
        ...
        if timeout 5s "${SPIRE_SERVER}" bundle show -socketPath "${SERVER_SOCKET}" > "${TRUST_BUNDLE_PATH}" 2>/dev/null; then
            ...
            for j in {1..10}; do
                ...
                if [ $j -eq 10 ]; then
                    ...
                    echo "    Server health: $(timeout 5s "${SPIRE_SERVER}" healthcheck -socketPath "${SERVER_SOCKET}" 2>&1 || echo "not responding")"
                fi
            done
        fi
    else
...
if [ -S "$SERVER_SOCKET" ] && [ -f "${SPIRE_DIR}/bin/spire-server" ]; then
    # First, clean up stale agents (agents from previous attestations that are no longer active)
    # This prevents parent ID mismatches when creating registration entries
    echo "  Cleaning up stale agents (keeping only current agent)..."
    
    # Disable exit-on-error for cleanup code (greps may return non-zero)
    set +e
    
    if "${SPIRE_DIR}/bin/spire-server" healthcheck -socketPath "$SERVER_SOCKET" >/dev/null 2>&1; then
        # Get the current agent ID from recent attestation logs
        CURRENT_AGENT_ID=""
        if [ -f /tmp/spire-server.log ]; then
            CURRENT_AGENT_ID=$(grep -E "Successfully reattested node|Successfully attested node" /tmp/spire-server.log 2>/dev/null | \
                tail -1 | \
                grep -oE 'spiffe://[^"]+' 2>/dev/null | \
                head -1 | \
                tr -d ' \r\n"')
        fi
        
        # List all agents and evict ones that don't match current
        AGENT_LIST=$("${SPIRE_DIR}/bin/spire-server" agent list -socketPath "$SERVER_SOCKET" 2>&1)
        if [ -z "$AGENT_LIST" ]; then
            AGENT_LIST=""
        fi
        AGENT_COUNT=$(echo "$AGENT_LIST" | grep -c "SPIFFE ID" 2>/dev/null)
        if [ -z "$AGENT_COUNT" ] || [ "$AGENT_COUNT" = "" ]; then
            AGENT_COUNT=0
        fi
        
        # IMPORTANT: Only clean up stale agents if we successfully identified the current one
        # Otherwise we might evict all agents!
        if [ "$AGENT_COUNT" -gt 1 ] && [ -n "$CURRENT_AGENT_ID" ]; then
            echo "  Found $AGENT_COUNT agents in database - cleaning up stale ones..."
            # Extract all agent SPIFFE IDs
            ALL_AGENT_IDS=$(echo "$AGENT_LIST" | grep -E "SPIFFE ID" 2>/dev/null | sed 's/.*SPIFFE ID[[:space:]]*:[[:space:]]*//' | tr -d '\r')
            EVICTED_COUNT=0
            while IFS= read -r agent_id || [ -n "$agent_id" ]; do
                agent_id=$(echo "$agent_id" | tr -d ' \r\n')
                if [ -n "$agent_id" ] && [ "$agent_id" != "$CURRENT_AGENT_ID" ]; then
                    # Evict the stale agent
                    if "${SPIRE_DIR}/bin/spire-server" agent evict -spiffeID "$agent_id" -socketPath "$SERVER_SOCKET" >/dev/null 2>&1; then
            ...
            if [ "$EVICTED_COUNT" -gt 0 ]; then
                echo -e "  ${GREEN}✓ Evicted $EVICTED_COUNT stale agent(s)${NC}"
            fi
        fi
    fi
...
if [ -f "${SCRIPT_DIR}/fetch-spire-bundle.py" ]; then
    ...
    if timeout 10s python3 "${SCRIPT_DIR}/fetch-spire-bundle.py" 2>&1; then
