...
def wait_for_agent_svid_in_logs(agent_log_path="/tmp/spire-agent.log", max_wait_seconds=300, check_interval=2):
    """
...
def fetch_from_workload_api_grpc(max_wait_seconds=300):
    """
    ...
    # Clean up URI prefix if present for file check
    if socket_path.startswith("unix://"):
        file_path = socket_path[7:]
    else:
    ...
    import time
    max_wait = 300
    print(f"Waiting for SPIRE Agent socket at {file_path}...")
    
    for i in range(max_wait):
        if os.path.exists(file_path):
            break
        # Check alternates if default not found
        if i > 5 and i % 5 == 0:
             for p in possible_paths:
                p_clean = p[7:] if p.startswith("unix://") else p
                if os.path.exists(p_clean):
                    print(f"Found socket at alternate location: {p_clean}")
                    socket_path = p
                    file_path = p_clean
                    break
        time.sleep(1)
    ...
    if not os.path.exists(file_path):
        print(f"Error: SPIRE Agent socket not found at {file_path}")
    ...
    try:
        ...
        # gRPC uses 'unix:' prefix for Unix domain sockets
        if socket_path.startswith("unix://"):
             # If it already has unix://, ensure it doesn't double up or malform
             # gRPC expects unix:/absolute/path for UDS
             # But our socket_path might be unix:///tmp... which is 3 slashes.
             # Typical grpc string: unix:///tmp/spire-agent/public/api.sock
             channel_target = socket_path
        else:
             channel_target = f'unix:{os.path.abspath(file_path)}'
        ...
        if os.path.exists(agent_log_path):
            ...
            import time
            max_entry_wait = 300  # Wait up to 30 seconds for entry to propagate
