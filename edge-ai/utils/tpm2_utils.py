...
class TPM2Utils:
    ...
    def sign_data(self, data: bytes, algorithm: str = "sha256") -> bytes:
        ...
        Returns:
            Signature
        """
        # Write data to appsig_info.bin file (as expected by the script)
        with open("appsig_info.bin", "wb") as msg_file:
            msg_file.write(data)
        
        # Get the directory of the script
        script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        sign_script = os.path.join(script_dir, "sign_app_message.sh")
        ...
        # Read the signature from appsig.bin (as generated by the script)
        if not os.path.exists("appsig.bin"):
            raise TPM2Error("Signature file appsig.bin not found after signing")
        
        with open("appsig.bin", "rb") as sig_file:
            signature = sig_file.read()
        
        # Cleanup temporary files
        if os.path.exists("appsig_info.bin"):
            os.unlink("appsig_info.bin")
        if os.path.exists("appsig.bin"):
            os.unlink("appsig.bin")
    ...
    def verify_signature(self, data: bytes, signature: bytes, algorithm: str = "sha256") -> bool:
        ...
        # Write signature to appsig.bin file (as expected by the script)
        with open("appsig.bin", "wb") as sig_file:
            sig_file.write(signature)
        
        # Get the directory of the script
        script_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        verify_script = os.path.join(script_dir, "verify_app_message_signature.sh")
