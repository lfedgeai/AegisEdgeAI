...
class TPM2Utils:
    ...
    def sign_data(self, data: bytes, algorithm: str = "sha256") -> bytes:
        ...
        Returns:
            Signature
        """
        # Write data to msg.bin file (as expected by the script)
        with open("msg.bin", "wb") as msg_file:
            msg_file.write(data)
        ...
        # Read the signature from sig.bin (as generated by the script)
        if not os.path.exists("sig.bin"):
            raise TPM2Error("Signature file sig.bin not found after signing")
        
        with open("sig.bin", "rb") as sig_file:
            signature = sig_file.read()
        
        # Cleanup temporary files
        if os.path.exists("msg.bin"):
            os.unlink("msg.bin")
    ...
    def verify_signature(self, data: bytes, signature: bytes, algorithm: str = "sha256") -> bool:
        ...
        # Write signature to sig.bin file (as expected by the script)
