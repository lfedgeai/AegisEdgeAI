...
class TPM2Utils:
    ...
    def sign_data(self, data: bytes, algorithm: str = "sha256") -> bytes:
        ...
        with open("tpm/appsig_info.bin", "wb") as msg_file:
            msg_file.write(data)
        ...
        if not os.path.exists("tpm/appsig.bin"):
            raise TPM2Error("Signature file tpm/appsig.bin not found after signing")
        
        with open("tpm/appsig.bin", "rb") as sig_file:
            signature = sig_file.read()
        
        # Cleanup temporary files
        if os.path.exists("tpm/appsig_info.bin"):
            os.unlink("tpm/appsig_info.bin")
        if os.path.exists("tpm/appsig.bin"):
            os.unlink("tpm/appsig.bin")
    ...
    def verify_signature(self, data: bytes, signature: bytes, algorithm: str = "sha256") -> bool:
        ...
        with open("tpm/appsig.bin", "wb") as sig_file:
